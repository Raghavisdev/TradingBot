...
class TestPipelineS6E2E(unittest.TestCase):
    ...
    @patch('engine.pipeline.trader')
    def test_pipeline_e2e_mcx_gt_2_accepted(self, mock_paper):
        ...
        # MCx = 3.0 — should still be accepted (no MCx gate)
        self.mock_recheck.return_value = ExecutionState(
        ...
        mock_paper.buy.return_value.entry_price = 3.0
        mock_paper.buy.return_value.entry_market_cap = 150000.0

        with patch.dict(os.environ, {"S6_Moonshot_VNext_MODE": "SHADOW", "LIVE_TRADING": "False"}):
            coin = process_message(message)
            
        # Verify NOT blocked by S6 gate — MCx > 2 should be accepted
        blocked = getattr(coin, 'buy_blocked_by', '') or ''
        self.assertNotIn('MCx', blocked, 'MCx gate should not exist in final baseline')
